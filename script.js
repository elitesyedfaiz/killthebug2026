...
defaultModerate['moderate_1'] = { title: "Target #01: Palindrome Check", desc: "Find the errors preventing this String check from functioning. Reward: +3 pts", points: 3, code: `import java.util.*;\n\npublic class PalindromeCheck {\n    public static void main(String[] args) {\n        String str = "madam";\n        String rev = "";\n        \n        for(int i = str.length(); i > 0; i--) {\n            rev = rev + str.charAt(i);\n        }\n        \n        if(str == rev) { \n            System.out.println("It is a Palindrome")\n        }\n    }\n}` };
defaultModerate['moderate_2'] = { title: "Target #02: Matrix Sum", desc: "This 2D Array diagonal sum crashes upon execution. Find the bug. Reward: +3 pts", points: 3, code: `public class MatrixSum {\n    public static void main(String[] args) {\n        int[][] mat = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}};\n        int sum = 0;\n        for(int i = 0; i <= mat.length; i++) {\n            sum += mat[i][i];\n        }\n        System.out.println("Sum: " + sum);\n    }\n}` };
defaultModerate['moderate_3'] = { title: "Target #03: Special Number", desc: "Checking if sum of factorials of digits == number. Find logic bugs. Reward: +3 pts", points: 3, code: `public class SpecialNumber {\n    public static void main(String[] args) {\n        int n = 145;\n        int temp = n;\n        int sum = 0;\n        while(temp > 0) {\n            int d = temp % 10;\n            int fact = 1;\n            for(int i = 1; i < d; i++) {\n                fact *= i;\n            }\n            sum += fact;\n            temp = temp / 10;\n        }\n        if(n = sum) {\n            System.out.println("Special Number");\n        }\n    }\n}` };

const db = {
    moderate: defaultModerate,
    easy: generateQuestions('Easy', 35, 2),
    hard: generateQuestions('Hardcore', 15, 5),
    surprise: { s1: { title: "🔥 BOSS: Surprise Question", desc: "The ultimate test of logic. Unlocked at T-minus 30 mins. Reward: +10 pts", points: 10, code: "// BOSS TARGET\npublic class Boss {\n}" } }
};
...
function executeSpin() {
    ...
    setTimeout(() => {
        ...
        // Match CSS exactly: Conic gradient starts at top (0 deg).
        // It rotates clockwise. The element at the top was originally at (360 - (randomDeg % 360)).
        // Slices: 0-36(R), 36-72(G), 72-108(R), 108-144(G)...
        let actualPos = (360 - (randomDeg % 360)) % 360;
        const isGreen = (actualPos % 72) >= 36;
        
        if (isGreen) {
            showModal("Result", "🟢 WHEEL LANDED ON GREEN!\n\nGrid converted to 35 EASY questions (+2 Marks).");
            localStorage.setItem('ktb_gridState', 'easy');
            localStorage.setItem('ktb_problems', JSON.stringify(db.easy));
        } else {
            showModal("Result", "🔴 WHEEL LANDED ON RED!\n\nGrid converted to 15 HARDCORE questions (+5 Marks).");
...
// --- 3. CARTOON LADYBUG ANIMATION & DEV CHEAT ---
...
// Developer Cheat Code: Spawn Golden Bug on F8
...
document.getElementById('login-form').addEventListener('submit', (e) => {
    ...
    if (type === 'admin') {
        ...
        localStorage.setItem('ktb_score', '16'); 
...
function startTimer() {
    ...
    function updateTick() {
        ...
        if (timeRemaining <= 20000 && timeRemaining > 0 && !bugRushActive) {
            triggerBugRush();
        }
...
function openShop() {
    ...
    let msg = `Buy a Hint Side-Quest for -${cost} Points?\n\nAlternatively, ask the invigilator for a FREE Physical Quest!`;
    
    // Fixed: Call showModal with 'confirm' instead of missing showConfirm
    showModal("Side Quest Shop", msg, 'confirm', () => {
...
function spawnGoldenBug() {
    ...
    bug.style.left = Math.random() * 80 + 'vw';
    bug.style.top = Math.random() * 80 + 'vh';
    
    // Append directly to body to bypass z-index issues!
    document.body.appendChild(bug);

    let moveInt = setInterval(() => {
        let nx = Math.random() * 90 + 'vw'; let ny = Math.random() * 90 + 'vh';
        bug.style.left = nx; bug.style.top = ny;
    }, 1000);

    bug.onclick = () => {
        clearInterval(moveInt); bug.remove();
        updateScore(2);
...
function triggerBugRush() {
    ...
    setInterval(() => {
        timeLeft--;
        if(timeLeft >= 0) document.getElementById('rush-timer').textContent = timeLeft;
    }, 1000);

    for(let i=0; i<10; i++) {
        let b = document.createElement('div');
        b.className = 'rush-bug';
        b.innerHTML = ladybugSVG;
...
function saveAdminProblem() {
    ...
    if (editingTargetId) {
        activeProblems[editingTargetId].title = title;
        activeProblems[editingTargetId].code = code;
    ...
    localStorage.setItem('ktb_problems', JSON.stringify(activeProblems));
    renderGrid(true);
    cancelAdminEdit(); 
}
...
function saveFixLocally() {
    ...
    if(selectedTypes.length === 0) {
        return showModal("Error", "You must select at least one Bug Type before saving!");
    }
    ...
    if (!submissions[currentProblemId] && localStorage.getItem('pName') !== "Admin") {
        updateScore(activeProblems[currentProblemId].points);
    }
